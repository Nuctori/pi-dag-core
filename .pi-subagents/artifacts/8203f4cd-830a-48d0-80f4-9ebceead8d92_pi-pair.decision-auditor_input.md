...
项目目录: C:\Users\Nuctori\pi-dag-core
决策链: C:\Users\Nuctori\pi-dag-core\.pi\decision-auditor\chain.md
对话日志: C:\Users\Nuctori\pi-dag-core\.pi\decision-auditor\convlog.md（只记用户提示与助手最终回复，供你推导目标与提取决策）
【会话隔离】convlog 由同一 cwd 下多个 pi 实例共享追加：本会话行 = 带 `<!--run:run-42864-p3b7d9-->` 标记的行，推导目标/提取决策只依据它们；无标记行（升级前历史/无法归属）仅作上下文理解、不得据此推导；其他 run 标记的行（其他实例的对话、审计者 run 自己的输出）一律忽略。
审计状态: C:\Users\Nuctori\pi-dag-core\.pi\decision-auditor\state.json（含签名状态、convExtractedLine 与 auditFindings）
...
2. 识别主 agent 实际做的关键决策（方案取舍/架构改动/采纳的用户要求），用 write **append** 到 C:\Users\Nuctori\pi-dag-core\.pi\decision-auditor\chain.md（## D-XXX: 标题 [Accepted]，Context/Decision/Rationale/Alternatives/Confidence/Date，编号 = 现有最大 D-NNN+1）。不记：命名、格式、单文件实现细节。
3. 用 write 更新 state.json：convExtractedLine 推进到最后一条对话行。无决策也推进。

【第二步：推导目标】
1. 用 read 读对话日志末尾（convlog 最后几条），从用户原话推导本轮任务目标（主 agent 自述不可信）。
2. 用 read 读过程日志（C:\Users\Nuctori\pi-dag-core\.pi\decision-auditor\process.md）——主 agent 的意图轨迹（决策信号摘要），对照它理解‘为什么这么做’，审产物时核对产物是否偏离了过程中表达的意图。
...
【收尾】用 write 更新 C:\Users\Nuctori\pi-dag-core\.pi\decision-auditor\state.json：inFlight=false，lastAuditedId 推进，lastAuditAt 置当前。产物通过 → signature={status:"passed"}、signatureConvLine 推进到 convlog 当前行；发现 blocker → signature={status:"blocked", blockers:[...具体可操作缺口]}、signatureConvLine 不推进。**保留 auditFindings（中间态历史不删）**。写完后立即停止。
