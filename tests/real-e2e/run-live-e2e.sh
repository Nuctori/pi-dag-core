#!/usr/bin/env bash
# REAL-model E2E on CI — no mock: a local Ollama model (free, open) drives a
# genuine pi session through the dag protocol. Proves: pi loads dag-core,
# a REAL LLM generates a spec, executes nodes via REAL subagent children,
# and the state machine completes with evidence.
set -euo pipefail
...
export LIVE_MODEL="${LIVE_MODEL:-qwen3:4b}"
...
echo "== run REAL pi with $LIVE_MODEL (no mock) =="
set +e
